...
func (self *SingleIP) Reset() {
	self.hasNext = true
}

func (self *SingleIP) HasNext() bool {
	if self.hasNext {
		self.hasNext = false
		return true
	}
...
func (self *SingleIP) Current() net.IP {
	return self.Addr
}
...
func ParseIPRange(raw string) (Range, error) {
	...
	if 2 != len(fields) {
		...
		if ipa != nil {
			if ipa.To4() == nil {
				return &SingleIP{hasNext: true, Addr: ipa}, nil
			}

			ip := binary.BigEndian.Uint32(ipa.To4())
			if 0 != ip {
